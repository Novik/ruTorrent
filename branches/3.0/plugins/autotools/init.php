...
if($needStart)
{
	...
	$cmdSchedule = new rXMLRPCCommand('schedule', array( 'autowatch', '10', $autowatch_interval."", 'execute={sh,-c,'.getPHP().' '.$pathToAutoTools.'/watch.php &}' ));
	$cmdInsert->addParameters( array('autolabel', 'branch=$not=$d.get_custom1=,"execute={'.getPHP().','.$pathToAutoTools.'/label.php,$d.get_hash=}"') );
	$cmdFinished->addParameters( array('automove', 'execute={'.getPHP().','.$pathToAutoTools.'/move.php,$d.get_hash=}') );
