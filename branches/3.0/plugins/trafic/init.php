...
if($do_diagnostic)
{
	if(!$pathToPHP || ($pathToPHP==""))
		findRemoteEXE('php',"thePlugins.get('trafic').showError('WUILang.trafPHPNotFound');",$remoteRequests);
	...
	if( (is_dir($st.'/trafic') && !isUserHavePermission($theSettings->uid,$theSettings->gid,$st.'/trafic',0x0007)) ||
		...
		$jEnd.="plugin.showError('WUILang.trafStatsNotAvailable');";
	...
	if(!isUserHavePermission($theSettings->uid,$theSettings->gid,$rootPath.'/plugins/trafic/update.php',0x0004))
		$jEnd.="plugin.showError('WUILang.trafUpdaterNotAvailable');";
}
$tm = getdate();
