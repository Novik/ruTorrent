...
var theRequestManager = 
{
	...
	{
		this[system].handlers.push( { ndx: this[system].commands.length, response: responseHandler } );
...
rTorrentStub.prototype.getpropsResponse = function(xml)
{
	...
	{
		handler.response( hash, ret, self.getValue(values,handler.ndx*2+1) );
	});
	return(ret);
}
...
rTorrentStub.prototype.gettotalResponse = function(xml)
{
	...
	{
		handler.response( ret, self.getValue(values,handler.ndx*2+1) );
	});
	return( ret );
}
...
rTorrentStub.prototype.getsettingsResponse = function(xml)
{
	...
	{
		handler.response( ret, self.getValue(values,i) );
		i+=2;
	});
...
rTorrentStub.prototype.getfilesResponse = function(xml)
{
	...
	for(var j=1;j<datas.length;j++)
	{
		...
		{
			handler.response( hash, fls, self.getValue(values,handler.ndx) );
		});
...
rTorrentStub.prototype.getpeersResponse = function(xml)
{
	...
	for(var j=1;j<datas.length;j++)
	{
		...
		{
			handler.response( id, peer, self.getValue(values,handler.ndx) );
		});
...
rTorrentStub.prototype.gettrackersResponse = function(xml)
{
	...
	for(var j=1;j<datas.length;j++)
	{
		...
		{
			handler.response( hash, trk, self.getValue(values,handler.ndx) );
		});
...
rTorrentStub.prototype.listResponse = function(xml)
{
	...
	for(var j=1;j<datas.length;j++)
	{
		...
		{
			handler.response( hash, torrent, self.getValue(values,handler.ndx) );
		});
...
function Ajax(URI, httpMethod, isASync, onComplete, onTimeout, onError, reqTimeout) 
{
