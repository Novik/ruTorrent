/*
 * WebUI - The WEB interface for uTorrent - http://www.utorrent.com
 * NO COPYCATS of language update
 * 
 * == BEGIN LICENSE ==
 * 
 * Licensed under the terms of any of the following licenses at your
 * choice:
 * 
 *  - GNU General Public License Version 2 or later (the "GPL")
 *    http://www.gnu.org/licenses/gpl.html
 * 
 *  - GNU Lesser General Public License Version 2.1 or later (the "LGPL")
 *    http://www.gnu.org/licenses/lgpl.html
 * 
 *  - Mozilla Public License Version 1.1 or later (the "MPL")
 *    http://www.mozilla.org/MPL/MPL-1.1.html
 * 
 * == END LICENSE ==
 * 
 * File Name: ua.js
 * 	Ukrainian language file.
 * 
 * File Author:
 * 		Artem Lopata (mod-s@yandex.ru)
 */
 
 var WUILang =
 {
 //all used
 kbs				: "кБ/с",
 
 
 //Settings window
 
 General			: "Загальне",
 Downloads			: "Завантаження",
 Connection			: "З'єднання",
 
 BitTorrent			: "BitTorrent",
 Queueing			: "Черга",
 Scheduler			: "Розклад",
 Advanced			: "Додатково",
 Disk_Cache			: "Дисковий кеш",
 Enable_Web_Interface : "Увімкнути веб інтерфейс",
 Authent			: "Аутентифікація",
 Username			: "Ім'я",
 Password			: "Пароль",
 Enable_Guest_acc	: "Увімк. гостоьвий акаунт з ніком",
 Connectivity		: "Зв'язок",
 Alt_list_port		: "Альтернативний порт (за зам. порт bittorrent)",
 Rest_access		: "Заборонити доступ наступним IP (Розділяти комою)",
 User_Interface		: "Інтерфейс користувача",
 Confirm_del_torr	: "Підтв. видалення торента",
 Update_GUI_every	: "Оновл. GUI кожні",
 ms					: "мс",
 Alt_list_bckgnd	: "Альтернативний колір списку",
 Show_speed			: "Показувати швидкість",
 Don_t				: "Ні",
 In_status_bar		: "У статус барі",
 In_title_bar		: "У заголовку",
 Show_cat_start		: "Показувати категорії при старті",
 Virt_row_thres		: "Поріг вірт. лінії",
 Show_det_start		: "Показувати деталі при старті",
 Restor_def			: "За замовчуванням",
 When_add_torrent	: "При додаванні торрента",
 Dnt_start_down_auto: "Не починати завантаження автоматично",
 Other_sett			: "Інші налаштовування",
 Append_ut_incompl	: "Дод. .!ut до незавершених торрентів",
 Prealloc_all_files	: "Виділити повний розмір файлу",
 Prev_stnd_w_act_con: "Не вмикати режим очікування якщо є активні торренти",
 Listening_Port		: "Порт",
 Port_f_incom_conns	: "Порт для вхідних з'єднань",
 Random_Port		: "Випадк. порт",
 Rnd_port_torr_start: "Випадковий порт при кожному відкритті rTorrent",
 En_UPnP_mapp		: "Увімк. UPnP порт мепінг",
 Add_torr_mustdie_f	: "Дод. rTorrent до винятків Windows Firewall (Windows XP SP2 та вище)",
 Proxy_Server		: "Проксі сервер",
 Type				: "Тип",
 none				: "(немає)",
 Proxy				: "Проксі",
 Port				: "Порт",
 Use_proxy_4_p2p_con: "Використовувати проксі для p2p з'єднань",
 Bandwidth_Limiting : "Глобальні обмеження швидкості",
 Global_max_upl		: "Макс. швидкість віддачі",
 unlimited			: "необмеж.",
 Alt_down_r_downl	: "Альтерн. швидкість віддачі якщо немає завантажень",
 Glob_max_downl		: "Макс. швидкість завантажень",
 Num_of_conn		: "Кільк. з'єднань",
 Glob_max_conn		: "Макс. кільк. з'эднань",
 Max_conn_peer_torr	: "Макс. кільк. з'єднаних пірів на торрент",
 Num_upl_slots		: "Кільк. слотів завантаження на торрент",
 Extra_ulslots		: "Використовувати дод. слоти віддачі, якщо швидкість віддачі < 90%",
 Add_bittor_featrs	: "Додаткові функціі BitTorrent",
 En_DHT_ntw			: "Увімкнути мережу DHT",
 Ask_scrape			: "Отр. інформ. (scrape) з трекера",
 En_DHT_new_torrs	: "Увімкнути DHT для нових торрентів",
 Peer_exch			: "Обмін пірами",
 Ip_report_track	: "IP/Hostname для передачі трекерові",
 Protocol_enc		: "Шифрування протоколу",
 Outgoing			: "Вихідні",
 Disabled			: "Відключено",
 Enabled			: "Увімкнуто",
 Forced				: "Форсовано",
 All_inc_legacy_conn: "Дозволити успадковані з'єднання",
 Queue_sett			: "Налаштування черги",
 Max_n_act_torrs	: "Макс. кільк. активних торрентів (завантаж. та віддачі)",
 Max_num_act_downl	: "Макс. кільк. активних завантажень",
 Seed_while			: "Розповсюджувати доки [значення за змовч.]",
 Ratio_is			: "Ратіо",
 or_time_is			: " % або час ",
 Ignore				: "(Ігнорувати)",
 nin_min			: "90 хвилин",
 two_h				: "2 години",
 three_h			: "3 години",
 four_h				: "4 години",
 five_h				: "5 годин",
 six_h				: "6 годин",
 seven_h			: "7 годин",
 eight_h			: "8 годин",
 nine_h				: "9 годин",
 ten_h				: "10 годин",
 twelve_h			: "12 годин",
 sixteen_h			: "16 годин",
 twenty_h			: "20 годин",
 tw_four_h			: "24 години",
 thirty_h			: "30 годин",
 thirty_six_h		: "36 годин",
 forty_eight_h		: "48 годин",
 sixty_h			: "60 годин",
 sev_two_h			: "72 години",
 nin_six_h			: "96 годин",
 Seed_prior			: "Розповсюджувані торренти мають більший приоритет",
 When_torr_reach	: "Коли rTorrent досягає цілі розповсюдження",
 Limit_upl_rate		: "Обмежити швидкість віддачі до [0 - зупинити]",
 En_shedul			: "Увімкнути розклад",
 Shed_sett			: "Налаштування розкладу",
 Lim_upl_rate		: "Обм. швидкість віддачі",
 Lim_dow_rate		: "Обм. швидкість завант.",
 Dis_DHT_when_t_off	: "Вимкнути DHT при відключенні",
 Disc_cahe			: "Кешування",
 Disc_cahe_bla_bla	: "Кэш використовується для утримання часто запрошуваних данних у пам'яті задля зменшення кількості звернень до жорсткого диску. rTorrent автоматично використовує кешування, але Ви можете налаштувати його змінюючи ці опції.",
 Overwr_d_cahe_au	: "Визначили розмір кешу",
 MB					: "MБ",
 Red_mem_usage		: "Зменшити використовування пам'яті коли кеш не потрібен",
 Adv_cache_sett		: "Додаткові налаштування кешу",
 En_cach_disc		: "Увімк. кеш зпису на диск",
 Wr_out_ever_2_min	: "Стирати пусті блоки кожні 2 хвилини",
 Wr_immed			: "Стирати закінчені блоки негайно",
 En_cah_disc_read	: "Увімк. кеш читання з диску",
 Turn_off_w_read	: "Вимк. кешування читання на диск, якщо швидкість завантаження мала",
 Rem_old_from_cahe	: "Видалити старі блоки з кешу",
 Increase_autom_cach: "Автом. збільшити розмір кешу, коли він заповниться",
 Advanced			: "Додатково",
 Advanced_label		: "Додаткові опції [ПОПЕРЕДЖЕННЯ: Не змінювати!!!]",
 Cancel				: "Відміна",
 uTorrent_settings	: "Налаштування rTorrent",
 
// Main window
 Doesnt_support		: "WebUI<sup>BETA</sup> не піддтримує Ваш браузер.",
 Name				: "Ім'я",
 Status				: "Статус",
 Size				: "Розмір",
 Done				: "Зроблено",
 Downloaded			: "Завантажено",
 Uploaded			: "Віддано",
 Ratio				: "Ратіо",
 DL					: "Завантаження",
 UL					: "Віддача",
 ETA				: "Час",
 Label				: "Мітка",
 Peers				: "Піри",
 Seeds				: "Розповсюджувачі",
 Avail				: "Дост.",
 Remaining			: "Залишилося",
 Priority			: "Приоритет",
 Download			: "Завантаження",
 Upload				: "Віддача",
 Not_torrent_file	: "Файл повинен бути файлом .torrent.",
 Pausing			: "Пауза",
 Seeding			: "Розповсюдження",
 Downloading		: "Завантаження",
 Checking			: "Перевірка",
 Error				: "Помилка",
 Queued				: "У черзі",
 Finished			: "Закінчено",
 Stopped			: "Зупинено",
 Request_timed_out	: "Час запросу до rTorrent війшов.",
 
 Force_Start		: "Форс. старт",
 Start				: "Старт",
 Pause				: "Пауза",
 Stop				: "Стоп",
 Force_recheck		: "Форс перевірка",
 New_label			: "Нова мітка...",
 Remove_label		: "Видалити мітку",
 Labels				: "Мітки",
 Remove				: "Видалити",
 Delete_data		: "Видалити данні",
 Remove_and			: "Видалити та",
 Details			: "Деталі...",
 Properties			: "Властивості...",
 of					: "з", 		//this two creates line 
 connected			: "з'єднані",	//  XX of YY connected
 High_priority		: "Високий приоритет",
 Normal_priority	: "Нормальний приоритет",
 Low_priority		: "Низький приоритет",
 Dont_download		: "Не завантажувати",
 Shure_restore_UI	: "Вы дійсно бажаєте відтворити інтерфейс користувача?",
 Reloading			: "Перевантаження...",
 Name				: "Ім'я",
 Date_				: "Дата",
 Files				: "Файли",
 Logger				: "Лог",
 
 s					: "с",    // part of "KB/s"
 bytes				: "байт",
 KB					: "KБ",
 MB					: "MБ",
 GB					: "ГБ",
 TB					: "ТБ",
 PB					: "ПБ",
 
 // main WND s
 Loading			: "Завантаження...",
 Torrent_file		: "Торрент файл",
 Torrent_URL		: "Торрент URL",
 Cookies			: "Cookies",
 RSS_Downloader		: "RSS завантажувач",
 Torrent_properties	: "Властивосты торрента",
 Trackers			: "Трекеры",
 Max_down_rate		: "Макс. швидкість завантаження",
 Max_upl_rate		: "Макс. швидкість віддачі",
 Bandwidth_sett		: "Налаштування обмежень",
 Number_ul_slots	: "Кільк. слотів віддачі",
 Override_default	: "Ігнорувати налаштування за зам.",
 Initial_seed		: "Супер seed",
 Enable_DHT			: "Увімк. DHT",
 Peer_ex			: "Обмін пірами",
 About				: "О программі",
 Enter_label_prom	: "Введіть нову мітку для виділених торрентів",
 Remove_torrents	: "Видалити торрент(и)",
 Rem_torrents_prompt: "Ви дійсно бажаєте відалити вибрані торрент(и)?",
 All				: "Усі",
 Active				: "Активні",
 Inactive			: "Не активні",
 No_label			: "Без мітки",
 Transfer			: "Передача",
 Time_el			: "Часу збігло",
 Remaining			: "Залишилося",
 Share_ratio		: "Ратіо",
 Down_speed			: "Швидкість завантаження",
 Down_limit			: "Обм. завантаження",
 Ul_speed			: "Швидкість віддачі",
 Ul_limit			: "Обм. віддачі",
 Wasted				: "Змарновано",
 Tracker			: "Трекер",
 Track_URL			: "URL трекеру",
 Track_status		: "Статус трекеру",
 Update_in			: "Оновити через",
 DHT_status			: "Статус DHT",
 Save_as			: "Зберегти як",
 Tot_size			: "Загальний розмір",
 Created_on			: "Створено",
 Comment			: "Коментар",
 
 
 //buttons
 add_button			: "Дод. файл",
 add_url			: "Дод. URL",
 ok1				: "   OK   ",
 ok2				: "   OK   ",
 ok3				: "   OK   ",
 Cancel1			: " Відміна ",
 Cancel2			: " Відміна ",
 no1				: "   Ні   ",
 
 mnu_add			: "Додати торрент...",
 mnu_remove			: "Видалити",
 mnu_start			: "Старт",
 mnu_pause			: "Пауза",
 mnu_stop			: "Стоп",
 mnu_rss			: "RSS Завантажувач",
 mnu_settings		: "Нлаштування...",
 mnu_search			: "Пошук...",
 mnu_lang			: "Мова",
 
  //Other variables added by spide
 torrent_add			: "Додати торрент", /*Caption of torrent add box*/
 time_w				: "w ", /*for x weeks*/
 time_d				: "d ", /*for x days remaining*/
 time_h				: "h ", /*for x hours remaining*/
 time_m				: "m ", /*for x minutes remaining*/
 time_s				: "s ", /*for x seconds remaining*/ 

 //Novik's addition
 Base_directory    		: "Directory",
 Number_Peers_min		: "Minimum number of peers",
 Number_Peers_max		: "Maximum number of peers",
 Tracker_Numwant		: "Wished number of peers",
 Number_Peers_For_Seeds_min	: "Minimum number of seeds",
 Number_Peers_For_Seeds_max	: "Maximum number of seeds",
 Enable_port_open		: "Open listening port",
 dht_port			: "UDP port to use for DHT",
 Ather_Limiting			: "Other limitations",
 Number_dl_slots		: "Number of download slots",
 Glob_max_memory		: "Maximum memory usage",
 Glob_max_files			: "Maximum number of open files",
 Glob_max_http			: "Maximum number of open http connections",
 Glob_max_sockets		: "Maximum number of open sockets",
 Ather_sett			: "Other",
 Directory_For_Dl		: "Default directory for downloads",
 Check_hash			: "Check hash after download",
 Hash				: "Hash",
 IsOpen				: "Open",
 DisableTracker			: "Disable",
 EnableTracker			: "Enable",
 ClientVersion			: "Client",
 Flags				: "Flags",
 ReqTimeout			: "Request timeout",
 GetTrackerMessage		: "Receive messages from tracker",
 Help				: "Help",
 PHPDoesnt_enabled		: "Your web server does not support PHP. Correct this and try again.",
 Speed				: "Speed",
 Dont_add_tname			: "Don't add torrent's name to directory", 
 Free_Disk_Space		: "Free disk space",
 badXMLRPCVersion		: "rTorrent is compiled with incorrect version of xmlrpc-c library, without i8 support. Version must be >= 1.11. Some functionality will be unavailable.",
 badLinkTorTorrent		: "Bad link to rTorrent. Check if it is really running. Check $scgi_port and $scgi_host settings in config.php and scgi_port in rTorrent configuration file.",
 badUploadsPath			: "Web server can't access torrents directory for read/write/execute. You can't add torrents through ruTorrent.",
 badSettingsPath		: "Web server can't access settings directory for read/write/execute. ruTorrent can't save own settings.",
 mnu_help			: "About...",
 badUploadsPath2		: "rTorrent's user can't access torrents directory for read/execute. You can't add torrents through ruTorrent.",
 View				: "View",
 AsList				: "as list",
 AsTree				: "as tree",
 Group				: "Group",
 SuperSeed			: "Super-seed",
 badTestPath			: "rTorrent's user can't access file ./test.sh for read/execute. ruTorrent will not work.",
 badSettingsPath2		: "rTorrent's user can't access settings directory for read/write/execute. ruTorrent will not work.",
 scrapeDownloaded		: "Downloaded",
 badSessionPath			: "Web server can't access rTorrent's session directory for read. ruTorrent will not work.",
 Total				: "Total",
 PCRENotFound			: "PHP module PCRE is not installed. ruTorrent will not work.",
 addTorrentSuccess		: "torrent is succesfully passed to rTorrent.",
 addTorrentFailed		: "Error: torrent is doesn't passed to rTorrent.",
 pnlState			: "State"
 };
