...
class PendingQueueTest extends TestCase
{
    ...
    private function fresh()
    {
        ...
        foreach (glob($this->tree . '/*.pending') as $f) {
            unlink($f);
        }
    ...
    private function hash($c)
    {
        return str_repeat($c, 40);
    }
    private function marker($h)
    {
        return $this->listPath . '/' . $h . '.pending';
    }
    ...
    public function testFiringAgainBeforeTheDrainIsNotASecondRequest()
    {
        ...
        $this->assertEquals(
            1,
            count(glob($this->listPath . '/*.pending')),
    ...
    public function testSomethingThatIsNotAHashNeverReachesTheFilesystem()
    {
        ...
        $escape = '../escaped';
        $this->assertTrue(
            erasedataQueueRequest($this->listPath, $escape, "1") === false,
            'a name that is not a hash is refused',
        );
        $this->assertTrue(
            !is_file($this->listPath . '/' . $escape . '.pending'),
            'and nothing was written outside the queue directory',
        );
        $this->assertEquals([], glob($this->listPath . '/*'), 'nor inside it');

        $this->assertTrue(
            erasedataQueueRequest($this->listPath, '../../etc/passwd', "1") === false,
    ...
    public function testFiringAgainDoesNotResetTheAttemptCount()
    {
        ...
        $h = $this->hash('3');
        erasedataQueueRequest($this->listPath, $h, "1");
        rXMLRPCRequest::$live = [$h];
        PendingProbe::$erasable = [];
        erasedataDrainOnce($this->listPath, [$h => "1"], 10);
    ...
    public function testAHashAlreadyCollectedLeavesTheQueue()
    {
        ...
        $pending = erasedataPendingHashes($this->listPath, []);
    ...
    public function testAMarkerThatIsNotAHashIsSweptUp()
    {
        ...
        erasedataPendingHashes($this->listPath, []);
        $this->assertTrue(
            !is_file($this->listPath . '/notahash.pending'),
    ...
    public function testAHashRtorrentNoLongerHoldsIsDroppedNotRetried()
    {
        ...
        $h = $this->hash('e');
        erasedataQueueRequest($this->listPath, $h, "1");
        rXMLRPCRequest::$live = [];          // the web UI removed it meanwhile
        erasedataDrainOnce($this->listPath, [$h => "1"], 10);
        $this->assertTrue(
            !is_file($this->marker($h)),
            'the marker goes rather than counting against the give-up limit',
        );
        $this->assertEquals([], PendingProbe::$erased, 'and rtorrent is not asked to erase it');
    }
    ...
    public function testAFailedEraseIsCountedAndRetriedLater()
    {
        ...
        erasedataQueueRequest($this->listPath, $h, "1");
        rXMLRPCRequest::$live = [$h];
        PendingProbe::$erasable = [];         // the erase does not take
        erasedataDrainOnce($this->listPath, [$h => "1"], 10);
    ...
    public function testItGivesUpRatherThanRetryingForever()
    {
        ...
        PendingProbe::$erasable = [];
        for ($i = 0; $i < 3; $i++) {
            erasedataDrainOnce($this->listPath, [$h => "1"], 3);
        }
        $this->assertTrue(
            !is_file($this->marker($h)),
    ...
    public function testALimitOfZeroNeverGivesUp()
    {
        ...
        PendingProbe::$erasable = [];
        for ($i = 0; $i < 5; $i++) {
            erasedataDrainOnce($this->listPath, [$h => "1"], 0);
        }
...
class PendingProbe
{
    ...
    public static $erasable = [];
    public static $erased = [];
}
