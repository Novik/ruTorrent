...
class Torrent
{
    ...
    {
        $fixture = is_array($source) ? $source : (self::$fixtures[$source] ?? ['errors' => true]);
        $this->hash = $fixture['hash'] ?? '';
        $this->info = $fixture['info'] ?? [];
        $this->hasErrors = !empty($fixture['errors']);
        $this->announceUrl = $fixture['announce'] ?? '';
        $this->announceList = $fixture['announce_list'] ?? [];
...
class CheckerTest
{
    public const SNAPSHOT_KEY = 'd.get_directory_base|d.get_custom1|d.get_throttle_name|d.get_connection_seed|d.get_state|d.is_open|d.stop|d.close';
    public const GETSTATE_KEY = 'd.get_custom|d.get_custom|d.get_custom|d.get_custom1';
    public const PREFLIGHT_KEY = 'd.get_custom|d.get_state|d.is_open';
    public const PREFLIGHT_KEY_COMMANDS = ['d.get_custom', 'd.get_state', 'd.is_open'];
    ...
    private function resetFakes()
    {
        ...
        FileUtil::$log = [];
        strictSetPrivateStatic('ruTrackerChecker', 'TRACKERS', []);
        strictSetPrivateStatic('ruTrackerChecker', 'ANNOUNCES', []);
    }

    // Fixtures for a replacement of hash OLD by hash NEW.
    private function stageTorrents($oldInfo = [], $newInfo = [])
    {
        Torrent::$fixtures['new-torrent'] = ['hash' => 'NEW', 'info' => $newInfo];
        rTorrent::$source = new Torrent(['hash' => 'OLD', 'info' => $oldInfo]);
    ...
    {
        $views = $views === null ? ['main', 'rat_2', 'rat_7', 'rat_9', 'rat_bad', 'rat_2_extra'] : $views;
        ...
        if ($existing !== false) {
            rXMLRPCRequest::queue(
                ...
                true,
                false,
                $existing === null ? array_merge(['main', 'default', 'seeding'], $views) : $existing,
            );
        }
    }
    ...
    {
        rXMLRPCRequest::queue(
            ['d.get_directory_base', 'd.get_custom1', 'd.get_throttle_name', 'd.get_connection_seed', 'd.get_state', 'd.is_open', 'd.stop', 'd.close'],
            true,
            false,
            [$baseDir, 'label', 'slow', 'seed-value', $state, $open, 0, 0],
        );
    }
    ...
    {
        rXMLRPCRequest::queue('d.hash', true, true, []);
        $this->queueViews();
    ...
    private function queueLoadConfirmed($marker = null)
    {
        rXMLRPCRequest::queue('d.get_custom', true, false, function () use ($marker) {
            return [$marker === null ? $this->currentReplacementMarker() : $marker];
        });
    }

    // Everything a committed replacement needs except the activation commands.
    private function stageHappyReplacement($baseDir, $state = 1, $open = 1, $oldInfo = [], $newInfo = [])
    {
        $this->stageTorrents($oldInfo, $newInfo);
        $this->queueTransactionStart($baseDir, $state, $open);
        $this->queueLoadConfirmed();
        rXMLRPCRequest::queue('d.erase', true, false, [0]);
        rXMLRPCRequest::queue('d.set_custom', true, false, []);
    }
    ...
    {
        strictInvoke('ruTrackerChecker', 'cleanupObsoleteFiles', [$oldTorrent, $newTorrent, $baseDir]);
    }
    ...
    public function testStartedReplacementSucceeds()
    {
        ...
        rXMLRPCRequest::queue('d.start', true, false, [0]);
        rXMLRPCRequest::queue(['d.get_state', 'd.is_open'], true, false, [1, 1]);
        ...
        strictAssertSame(
            ['view.set_visible=rat_2', 'view.set_visible=rat_7', 'view.set_visible=rat_9'],
        ...
        strictAssertSame(['NEW', 'chk-replacement', ''], $clears[0]['commands'][0]->params, 'the marker clear must target the new hash');
    }
    ...
    public function testMissingRatioViewIsForwardedAsAttributeOnly()
    {
        ...
        $this->withDebugLog(function () {
            ...
            rXMLRPCRequest::queue('d.hash', true, true, []);
            $this->queueViews(
                ['main', 'rat_2', 'rat_7', 'rat_9'],
                ['main', 'default', 'seeding', 'rat_2', 'rat_9'],  // rat_7 did not survive the restart
            );
    ...
    public function testUnreadableViewListForwardsEveryRatioViewAsAttributeOnly()
    {
        ...
        $this->withDebugLog(function () {
            $this->stageTorrents();
            rXMLRPCRequest::queue('d.hash', true, true, []);
            $this->queueViews(['main', 'rat_2', 'rat_9'], false); // nothing queued: view.list faults
            $this->queueSnapshot(sys_get_temp_dir(), 1, 1);
            $this->queueLoadConfirmed();
            rXMLRPCRequest::queue('d.erase', true, false, [0]);
            rXMLRPCRequest::queue('d.set_custom', true, false, []);
            ...
            strictAssertSame(
                ['d.views.push_back_unique=rat_2', 'd.views.push_back_unique=rat_9'],
                $this->membershipCommands(rTorrent::$lastSend['addition']),
    ...
    public function testNoRatioViewsSkipsTheViewListLookup()
    {
        ...
        $this->queueViews(['main', 'rat_bad', 'rat_2_extra'], false);
        $this->queueSnapshot(sys_get_temp_dir(), 1, 1);
        $this->queueLoadConfirmed();
        rXMLRPCRequest::queue('d.erase', true, false, [0]);
        rXMLRPCRequest::queue('d.set_custom', true, false, []);
        ...
        $keys = array_map(function ($request) {
            return $request['key'];
        }, rXMLRPCRequest::$requests);
        strictAssertTrue(
            !in_array('view_list', $keys, true),
    ...
    public function testStoppedOpenReplacementUsesOpen()
    {
        ...
        rXMLRPCRequest::queue('d.open', true, false, [0]);
        rXMLRPCRequest::queue(['d.get_state', 'd.is_open'], true, false, [0, 1]);
    ...
    public function testFullyStoppedReplacementSkipsActivation()
    {
        ...
        $this->stageHappyReplacement($base, 0, 0, ['name' => 'old.mkv'], ['name' => 'new.mkv']);
    ...
    public function testStartedReplacementMayRemainClosedWhileWaitingForSchedulerSlot()
    {
        ...
        rXMLRPCRequest::queue(['d.get_state', 'd.is_open'], true, false, [1, 0]);
    ...
    {
        $this->resetFakes();
        Torrent::$fixtures['new-torrent'] = ['hash' => 'NEW', 'info' => ['name' => 'new.mkv']];
        rXMLRPCRequest::queue('d.hash', true, false, ['NEW']);
        rXMLRPCRequest::queue(self::PREFLIGHT_KEY_COMMANDS, true, false, ['', 0, 0]);
        ...
        strictAssertSame(['NEW', 'chk-replacement'], $markerReads[0]['commands'][0]->params, 'the marker read must target the new hash');
    ...
    public function testLiveTorrentWithStaleMarkerIsNotAdopted()
    {
        ...
        rXMLRPCRequest::queue(self::PREFLIGHT_KEY_COMMANDS, true, false, ['stale-marker-from-dead-run', 1, 1]);
        rXMLRPCRequest::queue('d.set_custom', true, false, []);
        ...
        strictAssertSame(['NEW', 'chk-replacement', ''], $clears[0]['commands'][0]->params, 'the repair must clear the marker of the live torrent');
    ...
    public function testOrphanedStagedCopyIsAdoptedAndReplaced()
    {
        ...
        $this->stageTorrents();
        rXMLRPCRequest::queue('d.hash', true, false, ['NEW']);
        rXMLRPCRequest::queue(self::PREFLIGHT_KEY_COMMANDS, true, false, ['stale-marker-from-dead-run', 0, 0]);
        rXMLRPCRequest::queue('d.erase', true, false, [0]);
        $this->queueViews();
        $this->queueSnapshot(sys_get_temp_dir(), 1, 1);
        $this->queueLoadConfirmed();
    ...
    public function testCommitEraseWithUnknownOldStateLeavesStagedCopy()
    {
        ...
        $this->queueTransactionStart(sys_get_temp_dir(), 1, 1);
        $this->queueLoadConfirmed();
        rXMLRPCRequest::queue('d.erase', false, true, []);
    ...
    public function testSynchronousLoadFailureRestoresOldTorrent()
    {
        ...
        $this->stageTorrents(['name' => 'old.mkv'], ['name' => 'new.mkv']);
        rTorrent::$sendResult = false;
    ...
    public function testEraseRaceStillCompletesReplacement()
    {
        ...
        $this->queueTransactionStart(sys_get_temp_dir(), 1, 1);
        $this->queueLoadConfirmed();
        rXMLRPCRequest::queue('d.erase', true, true, []);
        rXMLRPCRequest::queue('d.hash', true, true, []);
        rXMLRPCRequest::queue('d.start', true, false, [0]);
        rXMLRPCRequest::queue(['d.get_state', 'd.is_open'], true, false, [1, 1]);
    ...
    public function testEraseFailureRestoresOldTorrentWithoutCleanup()
    {
        ...
        $this->queueLoadConfirmed();
        rXMLRPCRequest::queue('d.erase', true, true, []);
        rXMLRPCRequest::queue('d.hash', true, false, ['OLD']);
        rXMLRPCRequest::queue('d.erase', true, false, [0]);
        rXMLRPCRequest::queue('d.start', true, false, [0]);
    ...
    public function testActivationFailureAfterCommitStillFinishes()
    {
        ...
        $this->stageHappyReplacement($base, 1, 1, ['name' => 'old.mkv'], ['name' => 'new.mkv']);
    ...
    {
        $this->resetFakes();
        Torrent::$fixtures['not-a-torrent'] = ['errors' => true];
    ...
    {
        $this->resetFakes();
        rXMLRPCRequest::queue('d.set_custom|d.set_custom', true, true, []);
        rXMLRPCRequest::queue('d.hash', true, true, []);
    ...
    public function testDispatchPrefersCommentMatchesAndFallsBackToAnnounce()
    {
        $rows = [
            'comment match' => [
                ...
                'trackers' => [
                    ['/topic\.comment-test\.invalid/', '/tracker\.comment-test\.invalid/', 'comment-test'],
                ],
                'expect' => ['comment-test', 'https://topic.comment-test.invalid/view?id=42'],
            ],
            'announce fallback' => [
                'fixture' => [
                    'hash' => 'OLD',
                ...
                'trackers' => [
                    ['/topic\.announce-test\.invalid/', '/tracker\.announce-test\.invalid/', 'announce-test'],
                ],
                'expect' => ['announce-test', 'http://tracker.announce-test.invalid/announce'],
            ],
            'comment priority across handlers' => [
                'fixture' => [
                    'hash' => 'OLD',
                    'info' => ['name' => 'file.mkv'],
                ...
                'trackers' => [
                    ['/topic\.first-priority\.invalid/', '/tracker\.first-priority\.invalid/', 'first'],
                    ['/topic\.second-priority\.invalid/', '/tracker\.second-priority\.invalid/', 'second'],
                ],
                'expect' => ['second', 'https://topic.second-priority.invalid/view?id=42'],
            ],
            'announce-list flattening' => [
                'fixture' => [
                    'hash' => 'OLD',
                    'info' => ['name' => 'file.mkv'],
                    'announce' => 'http://unrelated.invalid/announce',
                    'announce_list' => [
                        ['http://unrelated-two.invalid/announce'],
                        ['http://tracker.list-test.invalid/announce'],
                    ],
                    'comment' => 'no topic URL here',
                ],
                'trackers' => [
                    ['/topic\.list-test\.invalid/', '/tracker\.list-test\.invalid/', 'list-test'],
                ],
                'expect' => ['list-test', 'http://tracker.list-test.invalid/announce'],
            ],
        ];
    ...
    public function testCleanupDeletesOnlyRenamedOldFileAndKeepsBase()
    {
        ...
        $old = new Torrent(['hash' => 'OLD', 'info' => ['files' => [
            ['path' => ['old', 'video-old.mkv'], 'length' => 3],
            ['path' => ['shared.nfo'], 'length' => 6],
        ]]]);
        $new = new Torrent(['hash' => 'NEW', 'info' => ['files' => [
            ['path' => ['video-new.mkv'], 'length' => 3],
            ['path' => ['shared.nfo'], 'length' => 6],
        ]]]);
    ...
    public function testCleanupDoesNotFollowDirectorySymlinkOutsideBase()
    {
        ...
        $old = new Torrent(['hash' => 'OLD', 'info' => ['files' => [
            ['path' => ['link', 'victim.mkv'], 'length' => 4],
        ]]]);
        $new = new Torrent(['hash' => 'NEW', 'info' => ['name' => 'replacement.mkv']]);
    ...
    public function testCleanupNeverRemovesBaseDirectory()
    {
        ...
        mkdir($base, 0777, true);
        file_put_contents($base . '/old.mkv', 'old');
        $old = new Torrent(['hash' => 'OLD', 'info' => ['name' => 'old.mkv', 'length' => 3]]);
        $new = new Torrent(['hash' => 'NEW', 'info' => ['name' => 'new.mkv', 'length' => 3]]);
    ...
    public function testCleanupRefusesFilesystemRootAsBase()
    {
        ...
        $old = new Torrent(['hash' => 'OLD', 'info' => ['files' => [
            ['path' => explode('/', ltrim($path, '/')), 'length' => 4],
        ]]]);
        $new = new Torrent(['hash' => 'NEW', 'info' => ['name' => 'replacement.mkv']]);
    ...
    public function testCleanupKeepsFileAliasedByNewTorrent()
    {
        ...
        $old = new Torrent(['hash' => 'OLD', 'info' => ['name' => 'b.mkv']]);
        $new = new Torrent(['hash' => 'NEW', 'info' => ['name' => 'a.mkv']]);
