...
class YouGetSignalParseTest extends TestCase
{
    ...
    {
        return json_encode(['data' => ['networkToolRunPortCheck' => [
            'output' => [
                'toolType' => 'PortCheck',
                'parsed' => [
                    'kind' => $kind,
    ...
    public function testAnOpenPortIsOpen()
    {
        $this->assertEquals(
            2,
            check_port_parse_yougetsignal($this->body('open')),
    ...
    public function testAClosedPortIsClosed()
    {
        $this->assertEquals(
            1,
            check_port_parse_yougetsignal($this->body('closed')),
    ...
    public function testAFilteredPortIsClosed()
    {
        ...
        $this->assertEquals(
            1,
            check_port_parse_yougetsignal($this->body('filtered')),
    ...
    public function testAScanThatDidNotSucceedIsUnknown()
    {
        $this->assertEquals(
            0,
            check_port_parse_yougetsignal($this->body('open', 'failure')),
    ...
    public function testAStateNobodyKnowsIsUnknown()
    {
        $this->assertEquals(
            0,
            check_port_parse_yougetsignal($this->body('unfiltered')),
    ...
    public function testSomethingThatIsNotJsonIsUnknown()
    {
        ...
        $this->assertEquals(
            0,
            check_port_parse_yougetsignal('<!DOCTYPE html><html></html>'),
