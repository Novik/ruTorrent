...
class XmppLibraryTest extends TestCase
{
    ...
    {
        $jid = new XMPPHP_XMLObj('jid', '', [], $fulljid);
        $bind = new XMPPHP_XMLObj('bind', 'urn:ietf:params:xml:ns:xmpp-bind');
        $bind->subs[] = $jid;
        $iq = new XMPPHP_XMLObj('iq', 'jabber:client', ['type' => 'result', 'id' => '1']);
        $iq->subs[] = $bind;
    ...
    public function testBindContinuesTheSessionHandshake()
    {
        ...
        $this->assertTrue(
            strpos($probe->sent[0], 'urn:ietf:params:xml:ns:xmpp-session') !== false,
    ...
    {
        $iq = new XMPPHP_XMLObj('iq', 'jabber:client', ['type' => 'error', 'id' => '1']);
        $probe = $this->newProbe();
        $probe->handleBind($iq);
    ...
    public function testBoshClassCanBeLoaded()
    {
        ...
        $this->assertTrue(
            strpos((string) $output, 'BOSH_LOADED') !== false,
