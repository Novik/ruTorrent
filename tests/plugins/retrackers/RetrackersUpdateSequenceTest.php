...
class RetrackersUpdateSequenceTest extends TestCase
{
    ...
    public function tearDown()
    {
        ...
        foreach ([$profile . '/settings', $profile] as $dir) {
            if (is_dir($dir)) {
                @rmdir($dir);
            }
        }
    ...
    private function retrackers($list, $todelete = [], $addToBegin = 0)
    {
    ...
    private function replay($torrent, $trks)
    {
            ...
            if (count($trks->list)) {
                if ($torrent->announce()) {
                    $torrent->announce_list($trks->addToBegin
                        ? array_merge($trks->list, [[$torrent->announce()]])
                        : array_merge([[$torrent->announce()]], $trks->list));
                } else {
        ...
        return [$wasAddition, $wasDeletion];
    }
    ...
    {
        $this->assertTrue(
            function_exists('clearTracker') && function_exists('deleteTrackers'),
    ...
    public function testAnnounceOnlyTorrentGainsAnAnnounceListEndingWithTheAddition()
    {
        ...
        list($wasAddition, $wasDeletion) = $this->replay(
            $torrent,
            $this->retrackers([['http://added.test/announce']]),
        );
        ...
        $expected = $this->bdict([
            ...
            'announce-list' => $this->bannounceList([
                ['http://one.test/announce'],
        ...
        $this->assertTrue(
            $reread->announce() === 'http://one.test/announce',
            'the original announce is untouched',
        );
        $this->assertTrue(
            $reread->announce_list() === [
                ['http://one.test/announce'],
                ['http://added.test/announce']],
    ...
    public function testAddToBeginPutsTheAdditionFirst()
    {
        ...
        list($wasAddition, $wasDeletion) = $this->replay(
            $torrent,
            $this->retrackers([['http://added.test/announce']], [], 1),
        );
    ...
    public function testATrackerlessTorrentGetsAnnounceAndAnnounceList()
    {
        ...
        list($wasAddition, ) = $this->replay($torrent, $this->retrackers([
            ['http://added.test/announce', 'http://added.test/announce2'],
            ['udp://added.test/announce'],
        ]));
        $this->assertTrue($wasAddition === true, 'a tracker was added');
    ...
    public function testATrackerAlreadyPresentIsNotAddedTwice()
    {
        ...
        $expected = $this->bdict([
            ...
            'announce-list' => $this->bannounceList([
                ['http://one.test/announce'],
                ['udp://two.test/announce', 'udp://two.test/announce2'],
                ['http://added.test/announce'],
            ]),
            'created by'    => $this->bstr($this->ourCreator()),
            'creation date' => $this->bint($this->stampedDate($written, $before)),
    ...
    public function testATorrentThatNeedsNothingIsNotRewritten()
    {
        ...
        list($wasAddition, $wasDeletion) = $this->replay(
            $torrent,
            $this->retrackers([['http://one.test/announce']]),
        );
    ...
    public function testAnEmptyConfigurationChangesNothing()
    {
        ...
        $torrent = new Torrent($fixture);
        list($wasAddition, $wasDeletion) = $this->replay($torrent, $this->retrackers([]));
    ...
    public function testDeletingEveryTrackerInAGroupRemovesTheGroup()
    {
        ...
        $expected = $this->bdict([
            'announce'      => $this->bstr('http://one.test/announce'),
            'announce-list' => $this->bannounceList([['http://one.test/announce']]),
            'created by'    => $this->bstr($this->ourCreator()),
            'creation date' => $this->bint($this->stampedDate($written, $before)),
            'info'          => $this->singleFileInfo(),
        ]);
        ...
        $reread = new Torrent($written);
        $this->assertTrue(
            $reread->announce_list() === [['http://one.test/announce']],
    ...
    public function testAdditionAndDeletionTogetherOnASessionTorrent()
    {
        ...
        list($wasAddition, $wasDeletion) = $this->replay(
            $torrent,
            $this->retrackers([['http://added.test/announce']], ['two.test']),
        );
    ...
    {
        $addition = [['http://a.test/x'], ['http://b.test/x', 'http://c.test/x']];
        $addition = clearTracker($addition, 'http://a.test/x');
        $this->assertEquals(
            [1 => ['http://b.test/x', 'http://c.test/x']],
            $addition,
    ...
    {
        $list = [['http://ONE.test/announce'], ['http://two.test/announce']];
        $this->assertTrue(
            deleteTrackers($list, ['one.TEST']) === true,
            'a case insensitive substring match is a deletion',
        );
        $this->assertEquals(
            [1 => ['http://two.test/announce']],
            $list,
            'and it is taken out of the list by reference',
        );
        $this->assertTrue(deleteTrackers($list, ['nothing.test']) === false,
