...
class EraseWithDataCommandTest extends TestCase
{
    ...
    public function testStopsAndClosesBeforeDeleting()
    {
        $this->assertEquals(
            0,
            strpos($this->command("1"), "d.stop=; d.close=; "),
    ...
    {
        $this->assertTrue(
            is_file(__DIR__ . '/../../../plugins/erasedata/erase.php'),
    ...
    {
        $this->assertTrue(
            strpos($this->command("1"), "execute.nothrow.bg={") !== false,
    ...
    {
        $this->assertTrue(
            strpos($this->command("1"), ',$' . getCmd("d.get_hash") . '=,') !== false,
    ...
    public function testForceFlagDistinguishesTheTwoActions()
    {
        ...
        $this->assertTrue(
            strpos($this->command("1"), $hash . ',1,') !== false,
            'Remove data asks for the download\'s own files',
        );
        $this->assertTrue(
            strpos($this->command("2"), $hash . ',2,') !== false,
