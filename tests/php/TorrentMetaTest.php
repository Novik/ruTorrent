...
class TorrentMetaTest extends TestCase
{
    ...
    public function testGettersReadTheKeysThatAreNotIdentifiers()
    {
        ...
        $this->assertTrue(
            $torrent->hash_info() === strtoupper(sha1($this->infoDictionary())),
    ...
    public function testClearRemovesTheKeyFromWhatIsWritten()
    {
        ...
        $this->assertTrue(
            strpos($written, $this->bstr('announce-list')) === false,
            'clear_announce_list() drops the announce-list key',
        );
        $this->assertTrue(
            strpos($written, $this->bstr('comment')) === false,
            'clear_comment() drops the comment key',
        );
        $this->assertTrue(
            strpos($written, $this->bstr('announce')) === false,
            'clear_announce() drops the announce key',
        );
        $this->assertTrue(
            strpos($written, $this->bstr('info')) !== false,
            'The rest of the dictionary is still written',
        );
        $this->assertTrue(
            is_null($torrent->announce()) && is_null($torrent->announce_list())
    ...
    public function testACallerCanBuildAKeyThatIsNotThereYet()
    {
        ...
        $this->assertTrue(
            !isset($torrent->libtorrent_resume),
            'The torrent no longer has libtorrent_resume',
        );
        $this->assertTrue(
            strpos((string) $torrent, $this->bstr('libtorrent_resume')) === false,
            'and it is not written',
        );

        $torrent->libtorrent_resume['bitfield'] = 7;
        $this->assertTrue(
            !isset($torrent->libtorrent_resume['files']),
            'isset() on an offset of a key answers for the offset',
        );
        $torrent->libtorrent_resume['files'] = [];
        $torrent->libtorrent_resume['files'][0] = ['priority' => 2, 'mtime' => 1234567891];
    ...
    public function testACallerCanIssetAndUnsetAKey()
    {
        ...
        $this->assertTrue(
            strpos((string) $torrent, $this->bstr('rtorrent')) === false,
            'An unset key is not written',
        );
        $torrent->rtorrent = ['directory' => '/downloads'];
        $this->assertTrue(
            (string) $torrent === $this->fixture(),
    ...
    public function testANumericKeyIsReadBackByEitherSpelling()
    {
        ...
        $this->assertTrue(
            $torrent->announce === 'http://a/announce',
    ...
    {
        $torrent = new Torrent($this->fixture());
        foreach (['errors', 'basedir', 'pointer', 'data', 'log_callback',
            'err_callback', 'filename', 'extra'] as $field) {
            $this->assertTrue(
                strpos((string) $torrent, $this->bstr($field)) === false,
    ...
    public function testMetaAlsoAnswersForTheKeysThatAreProperties()
    {
        $torrent = new Torrent($this->fixture());
        $this->assertTrue(
            $torrent->meta('announce') === $torrent->announce,
        ...
        $this->assertTrue(
            is_null($torrent->comment) && !isset($torrent->comment),
            'clearMeta() drops a key that is a property',
        );
        $this->assertTrue(
            strpos((string) $torrent, $this->bstr('comment')) === false,
    ...
    public function testSettersWriteTheKeyAndStampTheCreator()
    {
        ...
        $before = time();
        $this->assertTrue(
            is_null($torrent->announce('http://new.test/announce')),
        ...
        $this->assertTrue(
            is_null($torrent->url_list(['http://new.test/f.dat'])),
            'The url_list() setter returns null',
        );
        $this->assertTrue(
            is_null($torrent->announce_list([['http://new.test/announce']])),
            'The announce_list() setter returns null',
        );
        $this->assertTrue(
            is_null($torrent->httpseeds(['http://new.test/f.dat'])),
    ...
    public function testReadingAKeyThatIsNotThereDoesNotAddIt()
    {
        ...
        $this->assertTrue(
            (string) $empty === 'd4:infod4:name1:xee',
    ...
    public function testMetadataIsNotStoredAsDynamicObjectProperties()
    {
        ...
        $this->assertEquals(
            [],
            $this->dynamicProperties($torrent),
