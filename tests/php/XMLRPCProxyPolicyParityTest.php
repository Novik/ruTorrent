...
class XMLRPCProxyPolicyParityTest extends TestCase
{
    ...
    {
        $this->assertTrue(
            is_array($this->reference) && (count($this->reference) > 0),
        ...
        foreach (['d.open', 'd.close', 'd.start', 'd.stop'] as $action) {
            $this->assertTrue(
                in_array($action, (array) $this->reference, true),
