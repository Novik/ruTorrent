...
class EnvCheckShippedConfigTest extends TestCase
{
    ...
    private function shippedConfig()
    {
        ...
        $saved = $_ENV;
        foreach (['RU_LOG_FILE', 'RU_TOP_DIR', 'RU_SCGI_PORT', 'RU_SCGI_HOST',
    ...
    public function testTheShippedLogFileIsOneTheCheckerAccepts()
    {
        ...
        $this->assertTrue(
            Requirements::logFilePathValid($cfg['log_file']),
    ...
    public function testTheShippedMountPointIsOneTheCheckerAccepts()
    {
        $cfg = $this->shippedConfig();
        $this->assertTrue(
            Requirements::xmlrpcMountPointValid($cfg['XMLRPCMountPoint']),
    ...
    public function testTheShippedTopDirectoryIsAbsolute()
    {
        $cfg = $this->shippedConfig();
        $this->assertTrue(
            Requirements::looksAbsolute($cfg['topDirectory']),
    ...
    {
        $masks = ['0777', '777', '0770', '0700', '02770', '1777', '0666x', 'garbage', '8'];
    ...
    public function testTheCheckerReportsAMaskItCouldNotRead()
    {
        ...
        $this->assertTrue(
            strpos($text, 'RU_PROFILE_MASK') !== false,
            'the checker mentions the mask it could not read',
        );
        $this->assertTrue(
            strpos($text, '[WARN') !== false,
