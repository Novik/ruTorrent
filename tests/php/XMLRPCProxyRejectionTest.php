...
{
    private $safe = ['d.custom1.set', 'd.directory.set'];
    private $opts;
    ...
    public function setUp()
    {
        ...
        $this->opts = ['directory' => ['root' => '/', 'resolve' => null]];
    }
    ...
    {
        $d = $this->call('execute.capture', ['', '/bin/id']);
        $this->assertTrue(
            $d['action'] === 'reject',
            'execute.capture is refused',
        );
        $this->assertTrue(
            isset($d['method']) && $d['method'] === 'execute.capture',
    ...
    public function testSystemShutdownIsRejectedAndNamed()
    {
        ...
        $this->assertTrue(
            isset($d['method']) && $d['method'] === 'system.shutdown',
    ...
    public function testAMulticallCarryingADeniedCommandNamesThatCommand()
    {
        ...
        $d = $this->call('d.multicall2', ['', 'main', 'execute.capture=/bin/id']);
        $this->assertTrue(
            $d['action'] === 'reject',
    ...
    {
        $this->assertTrue(
            method_exists('XMLRPCProxy', 'rejectionMessage'),
    ...
    {
        $this->assertTrue(
            method_exists('XMLRPCProxy', 'rejectionFault'),
        ...
        $this->assertTrue(
            strpos($fault, 'Is rTorrent running') === false,
