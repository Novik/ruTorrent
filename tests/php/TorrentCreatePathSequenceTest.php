...
class TorrentCreatePathSequenceTest extends TestCase
{
     ...
     * class.
     */
    private function createdInfo($extra = [])
    {
        $order = ['sub/b.data', 'a.data'];
        $stream = '';
        $entries = [];
    ...
    public function testCreatingATorrentWritesEveryKeyTheFormCanSet()
    {
        $before = time();
        $announce_list = [
            ['http://one.test/announce', 'http://one.test/announce2'],
        ...
        $this->assertTrue(
            $torrent->hash_info() === strtoupper(sha1($this->createdInfo([
                'private' => $this->bint(1),
                'source'  => $this->bstr('XIRVIK'),
            ]))),
    ...
    {
        $before = time();
        $torrent = new Torrent($this->source, [], 16, null, null);
    ...
    public function testASavedTorrentReadsBackByteForByte()
    {
        ...
        $this->assertTrue(
            file_get_contents($this->saved) === $written,
        ...
        $this->assertTrue(
            strpos($written, $this->bstr('created by') . $this->bstr($this->ourCreator())) !== false,
            "'created by' is written with our creator",
        );
        $this->assertTrue(
            strpos($written, $this->bstr('creation date') . $this->bint($stamped)) !== false,
            "'creation date' is written with the time of the build",
        );
        $this->assertTrue(
            $reread->announce_list() === [
                ['http://one.test/announce'], ['udp://two.test/announce']],
    ...
    public function testCorrectingAnExternallyBuiltTorrentSetsTheFormsValues()
    {
        ...
        $announce_list = [['http://corrected.test/announce']];
        $trackersCount = 1;
