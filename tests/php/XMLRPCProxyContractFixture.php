...
return [
    "off mode rejects and sends nothing" => [
    ...
    "off mode rejects a body that is not XML" => [
    ...
    "passthrough_unsafe forwards the body verbatim, trusted" => [
    ...
    "a body that is not XML is forwarded untrusted" => [
        "request" => "not xml at all",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [
    ...
    "a methodCall with no methodName is forwarded untrusted" => [
    ...
    "an unknown method is forwarded untrusted" => [
    ...
    "a newline in an unknown method name cannot forge a log line" => [
    ...
    "load.start with an allowed command param is rebuilt and trusted" => [
    ...
    "load.start strips a command param that is not allowed" => [
    ...
    "every stripped param is named in one log line" => [
    ...
    "a param this side cannot rebuild forces the call untrusted" => [
    ...
    "a base64 data param is re-emitted without its line wrapping" => [
    ...
    "a value with no explicit string element is read the same way" => [
    ...
    "the 0.9.x method names take the same parameter positions" => [
    ...
    "a command taking two arguments keeps both, each trimmed" => [
    ...
    "quotes and backslashes in a value are escaped, not dropped" => [
    ...
    "an argument starting with \$ is dropped rather than quoted" => [
    ...
    "a value the client quoted itself is kept as one argument" => [
    ...
    "a long stripped value is truncated in the log" => [
    ...
    "a newline in a stripped value cannot forge a log line" => [
    ...
    "an empty allowlist strips every command param" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>load.start</methodName><params><param><value><string></string></value></param><param><value><string>http://example.test/x.torrent</string></value></param><param><value><string>d.custom1.set=label</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [],
    ...
    "a load call with no params at all is still rebuilt" => [
    ...
    "logging off changes what is logged and nothing else" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>load.start</methodName><params><param><value><string></string></value></param><param><value><string>http://example.test/x.torrent</string></value></param><param><value><string>execute=evil</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => false,
        "safeParams" => [
    ...
    "logging off on the unknown-method path too" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>system.client_version</methodName><params></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => false,
        "safeParams" => [
    ...
    "a multicall of read commands is forwarded untouched and untrusted" => [
    ...
    "a multicall whose commands are all allowed is rebuilt and trusted" => [
    ...
    "one unrebuildable command sends the whole multicall untouched" => [
    ...
    "a multicall carrying execute.capture is forwarded untouched and untrusted" => [
    ...
    "an allowed command with a \$ argument makes the multicall untrusted" => [
    ...
    "a chained command stays inside the argument it was quoted into" => [
    ...
    "a multicall view name is data, never a command" => [
    ...
    "a data param that cannot be rebuilt makes the multicall untrusted" => [
    ...
    "d.multicall takes commands in the same position" => [
    ...
    "d.multicall.filtered takes commands in the same position" => [
    ...
    "t.multicall is command-carrying too" => [
    ...
    "f.multicall is command-carrying too" => [
    ...
    "p.multicall is command-carrying too" => [
    ...
    "system.multicall is not one of them" => [
    ...
    "load.start from a local path is rejected" => [
    ...
    "load.normal from a local path is rejected" => [
    ...
    "the 0.9.x name is rejected the same way" => [
    ...
    "a relative path is a local path too" => [
    ...
    "a tilde path is a local path too" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>load.start</methodName><params><param><value><string></string></value></param><param><value><string>~/watch/x.torrent</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [
    ...
    "an uppercase scheme is a local path to rtorrent, so it is rejected" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>load.start</methodName><params><param><value><string></string></value></param><param><value><string>HTTP://example.test/x.torrent</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [
    ...
    "magnet without the ? is a local path to rtorrent, so it is rejected" => [
    ...
    "an ftp url is accepted, as rtorrent accepts it" => [
    ...
    "load.raw_start is unaffected \xE2\x80\x94 its parameter is the torrent, not a URI" => [
    ...
    "a local path is allowed when the operator turns it on" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>load.start</methodName><params><param><value><string></string></value></param><param><value><string>/srv/watch/x.torrent</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [
    ...
    "the 0.9.8 spelling execute2 is refused by the same prefix" => [
    ...
    "schedule_remove2 is refused by the same prefix" => [
    ...
    "a multicall carrying a refused command is refused, not forwarded" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>d.multicall2</methodName><params><param><value><string></string></value></param><param><value><string>main</string></value></param><param><value><string>execute.capture=/bin/sh</string></value></param></params></methodCall>",
        "mode" => "sanitize",
        "enableLog" => true,
        "safeParams" => [
    ...
    "system.multicall carrying a refused member is refused" => [
    ...
    "system.multicall of harmless members is still forwarded untrusted" => [
    ...
    "passthrough_unsafe is not subject to the refusal list" => [
        "request" => "<?xml version=\"1.0\"?><methodCall><methodName>execute.capture</methodName><params><param><value><string></string></value></param><param><value><string>/bin/sh</string></value></param></params></methodCall>",
        "mode" => "passthrough_unsafe",
        "enableLog" => true,
        "safeParams" => [
    ...
    "d.start on one hash is elevated" => [
    ...
    "d.stop on one hash is elevated" => [
    ...
    "d.open on one hash is elevated" => [
    ...
    "a label is elevated and the value is carried as data" => [
    ...
    "a \$ value on an elevated setter is data, not a command" => [
    ...
    "d.priority.set takes a hash and a number" => [
    ...
    "d.delete_tied is elevated on a hash" => [
    ...
    "a hash-shaped argument is required" => [
    ...
    "an elevated method with the wrong argument count is not elevated" => [
    ...
    "the xmlrpc size limit is elevated but clamped" => [
    ...
    "a size under the ceiling is passed through" => [
