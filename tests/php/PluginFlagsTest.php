...
class PluginFlagsTest extends TestCase
{
    ...
    public function testAFlagNobodySetIsTrue()
    {
        ...
        $this->assertEquals(
            true,
            getFlag($p, 'ratio', 'enabledByDefault'),
            'a flag missing from the plugin section is true',
        );
        $this->assertEquals(
            true,
            getFlag($p, 'nosuchplugin', 'enabledByDefault'),
            'a plugin missing from the file entirely is true',
        );
        $this->assertEquals(
            true,
            (bool) getFlag($p, 'ratio', 'enabledByDefault'),
    ...
    {
        foreach (['no', 'off', 'false', '0'] as $written) {
    ...
    {
        foreach (['yes', 'on', 'true', '1'] as $written) {
            $p = $this->parse("[ratio]\nenabledByDefault = $written\n");
            $this->assertEquals(
                true,
                (bool) getFlag($p, 'ratio', 'enabledByDefault'),
    ...
    public function testTheDefaultSectionAnswersForPluginsThatDoNotSayOtherwise()
    {
        ...
        $this->assertEquals(
            false,
            (bool) getFlag($p, 'rss', 'enabledByDefault'),
            'a plugin with no opinion of its own takes the [default] one',
        );
        $this->assertEquals(
            false,
            (bool) getFlag($p, 'unlisted', 'enabledByDefault'),
