...
class SocketAllocLimitsTest extends TestCase
{
    ...
    {
        $this->assertTrue(
            rTorrentSettings::socketAllocSupported(0x1015),
            '0.16.21 reports its socket allocation limits',
        );
        $this->assertTrue(
            rTorrentSettings::socketAllocSupported(0x1016),
            'and so does anything after it',
        );
        $this->assertTrue(
            !rTorrentSettings::socketAllocSupported(0x1014),
            '0.16.20 does not, and must not be asked',
        );
        $this->assertTrue(
            !rTorrentSettings::socketAllocSupported(0x908),
    ...
    public function testAnUnknownDaemonIsNotAsked()
    {
        ...
        $this->assertTrue(
            !rTorrentSettings::socketAllocSupported(0),
            'the unreachable-daemon sentinel is not a version that answers',
        );
        $this->assertTrue(
            !rTorrentSettings::socketAllocSupported(null),
    ...
    public function testTheBudgetIsWhatTheOfferedCategoriesShare()
    {
        ...
        $this->assertTrue(
            $settings->applySocketAllocLimits([65536, 1024, 1448, 4096, 65536, 4]),
            'a well-formed answer is taken',
        );
        $this->assertEquals(
            63064,
            $settings->socketAllocBudget,
    ...
    {
        $settings = $this->settings();
        $settings->applySocketAllocLimits([1024, 2048, 512, 4096, 65536, 4]);
        $this->assertEquals(
            0,
            $settings->socketAllocBudget,
    ...
    public function testAnAnswerOfTheWrongShapeLeavesTheDefaults()
    {
        $settings = $this->settings();
        $this->assertTrue(
            !$settings->applySocketAllocLimits([65536, 1024, 1448]),
    ...
    public function testAnAnswerThatIsNotNumbersLeavesTheDefaults()
    {
        $settings = $this->settings();
        $this->assertTrue(
            !$settings->applySocketAllocLimits(['n/a', 1024, 1448, 4096, 65536, 4]),
