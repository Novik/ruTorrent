...
class Requirements
{
    public const MIN_PHP = '7.4.0'; // arrow functions (fn() =>) are used in the codebase
    ...
    {
        $ver = (string) $ver;
        if (preg_match('/^0\.9\.8(\D|$)/', $ver)) {
            return [true, 'supported (0.9.8 baseline)'];
        }
        if (preg_match('/^0\.16\./', $ver)) {
            return [true, 'supported (0.16.x series)'];
        }
        return [false, 'not an explicitly supported version (0.9.8 or 0.16.x) -- may or may not work'];
    }
    ...
    {
        $builtin = ['1_prepare', '~_save_full'];
        return is_array($keys) ? array_values(array_diff($keys, $builtin)) : [];
    }
    ...
    public static function scgiConfigured($host, $port)
    {
        ...
        if (self::isUnixSocket($host)) {
            return strlen(self::unixSocketPath($host)) > 0;
        }
    ...
    public static function pluginExtensions($info)
    {
        ...
        foreach (preg_split('/\r\n|\r|\n/', $info) as $line) {
            if (!preg_match('/^\s*php\.extensions\.(error|warning)\s*:\s*(.+)$/', $line, $m)) {
                continue;
            }
            foreach (explode(',', $m[2]) as $ext) {
                $ext = trim($ext);
                if ($ext !== '' && !in_array($ext, $ret[$m[1]], true)) {
                    $ret[$m[1]][] = $ext;
                }
            }
        }
    ...
    public static function missingPluginExtensions($byPlugin, $loaded)
    {
        ...
        foreach ($byPlugin as $plugin => $needs) {
            foreach (['error', 'warning'] as $severity) {
                foreach ((isset($needs[$severity]) ? $needs[$severity] : []) as $ext) {
                    if (!isset($wanted[$ext])) {
                        $wanted[$ext] = [];
                    }
        ...
        foreach ($wanted as $ext => $plugins) {
            if (in_array(strtolower($ext), $have, true)) {
                continue;
            }
            ksort($plugins);
            $ret[$ext] = [
                'disables' => array_keys(array_filter($plugins, function ($s) {
                    return $s === 'error';
                })),
                'limits'   => array_keys(array_filter($plugins, function ($s) {
                    return $s === 'warning';
                })),
            ];
        }
    ...
    {
        $scheme = self::logFileStreamScheme($path);
        if ($scheme === null || !is_array($wrappers)) {
            return false;
        }
        ...
        $builtin = ['https', 'ftps', 'compress.zlib', 'php', 'file',
...
if (defined('RUTORRENT_REQUIREMENTS_LIB')) {
    return;
}
...
$results = []; // each: [section, ok|null, label, detail]  (ok===null => informational)
function check($section, $ok, $label, $detail = '')
{
    global $results;
    $results[] = [$section, $ok, $label, $detail];
}
...
if ($cfg === null) {
    ...
    check(
        'config',
        Requirements::scgiConfigured($cfg['scgi_host'], $cfg['scgi_port']),
        '$scgi_host / $scgi_port',
        'rtorrent SCGI address: ' . Requirements::scgiLabel($cfg['scgi_host'], $cfg['scgi_port']),
    );
    check(
        'config',
        Requirements::xmlrpcMountPointValid($cfg['mount']),
        '$XMLRPCMountPoint',
        'must be set (config warns not to delete it); found: ' . var_export($cfg['mount'], true),
    );
    if ($cfg['topdir'] !== null) {
...
if (@is_dir($pluginDir)) {
    ...
    foreach ($missing as $ext => $cost) {
        $detail = [];
        if ($cost['disables']) {
            $detail[] = 'disables ' . implode(', ', $cost['disables']);
        }
        if ($cost['limits']) {
            $detail[] = 'limits ' . implode(', ', $cost['limits']);
        }
...
function scgi_call($host, $port, $methodName, $params = [], $asList = false)
{
    ...
    if ($asList) {
        if (preg_match_all('#<string>(.*?)</string>#s', $resp, $m)) {
            return $m[1];
        }
        return [];
    }
    if (preg_match('#<value>\s*<string>(.*?)</string>#s', $resp, $m)) {
        return $m[1];
    }
    if (preg_match('#<string>(.*?)</string>#s', $resp, $m)) {
        return $m[1];
    }
    return null;
}
...
$sections = ['req' => 'Required', 'rec' => 'Recommended', 'config' => 'Configuration', 'plugins' => 'Plugins', 'rtorrent' => 'rtorrent'];
$lines = [];
