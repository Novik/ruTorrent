...
class commonEngine
{
    ...
    public static function removeTags($s, $charset = "UTF-8")
    {
        return(html_entity_decode(str_replace("&nbsp;", " ", strip_tags($s)), ENT_QUOTES, $charset));
    }
    public static function formatSize($item)
    {
    ...
    public static function fromUTF($out, $encoding)
    {
...
class rSearchHistory
{
    ...
    {
        $this->lst[$url] = [ "hash" => $hash, "time" => time() ];
        $this->changed = true;
    }
    ...
    {
        uasort($this->lst, ["Utility", "sortArrayTime"]);
...
class engineManager
{
        ...
        if ($handle = opendir($dir)) {
            while (false !== ($file = readdir($handle))) {
                if (is_file($dir . '/' . $file)) {
                    ...
                    $this->engines[$name] = [ "name" => $name, "path" => FileUtil::fullpath($dir . '/' . $file), "object" => $name . "Engine", "enabled" => true, "global" => true, "limit" => 100 ];
    ...
    protected static function sortBySeeds($a, $b)
    {
    ...
    public function action($eng, $what, $cat = "all")
    {
        ...
        uasort($arr, [__CLASS__,"sortBySeeds"]);
        $cnt = 0;
        $history = self::loadHistory(true);

        $ret = [ "eng" => $eng, "cat" => $cat, "data" => [] ];
