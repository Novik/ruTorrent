...
class NNMClubCheckImpl
{
    ...
    {
        if (preg_match('`^https?://(nnm-club|nnmclub)\.(ru|me|to|name)/forum/viewtopic\.php\?p=(?P<id>\d+)$`', $url, $matches)) {
            $client = ruTrackerChecker::makeClient("https://nnm-club.name/forum/viewtopic.php?p=".$matches["id"]);
