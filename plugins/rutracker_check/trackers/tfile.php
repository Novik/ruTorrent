...
class TfileCheckImpl
{
    ...
    {
        if (preg_match('`^https?://tfile\.(me|co)/forum/viewtopic\.php\?p=(?P<id>\d+)$`', $url, $matches)) {
            $client = ruTrackerChecker::makeClient("http://tfile.co/forum/viewtopic.php?p=".$matches["id"]);
