...
class TfileCheckImpl
{
    ...
    {
        if (preg_match('`^https?://tfile\.(me|co|cc)/forum/viewtopic\.php\?p=(?P<id>\d+)$`', $url, $matches)) {
            $client = ruTrackerChecker::makeClient("http://tfile-video.org/forum/viewtopic.php?p=".$matches["id"]);
            ...
            if (preg_match('`Info hash:</td><td><strong>(?P<hash>[0-9A-Fa-f]{40})</strong></td>`', $client->results, $matches)) {
                ...
                if (preg_match('`\"download.php\?id=(?P<id>\d+)`', $client->results, $matches)) {
                    $client->setcookies();
                    $client->fetchComplex("http://tfile-video.org/forum/download.php?id=".$matches["id"]);
...
ruTrackerChecker::registerTracker("/tfile\./", "/tfile\.|peersteers\.org/", "TfileCheckImpl::download_torrent");
