...
class ruTrackerChecker
{
    ...
    public const MAX_LOCK_TIME		= 900;	// 15 min
    ...
    public const LOAD_WAIT_ATTEMPTS	= 40;
    public const LOAD_WAIT_DELAY_US	= 50000;
    public const REPLACEMENT_MARKER_KEY	= 'chk-replacement';
    ...
    public static function supportedTrackers()
    {
     ...
     */
    protected static function torrentExists($hash)
    {
    ...
    protected static function setState($hash, $state)
    {
        $req = new rXMLRPCRequest([
            new rXMLRPCCommand(getCmd("d.set_custom"), [$hash, "chk-state", $state . ""]),
            new rXMLRPCCommand(getCmd("d.set_custom"), [$hash, "chk-time", time() . ""]),
        ]);
        if ($state == self::STE_UPTODATE) {
            $req->addCommand(new rXMLRPCCommand(getCmd("d.set_custom"), [$hash, "chk-stime", time() . ""]));
        }
        $req->important = false;
        if ($req->success()) {
            return(true);
    ...
    protected static function getState($hash, &$state, &$time, &$successful_time, &$label)
    {
        ...
        $req = new rXMLRPCRequest([
            new rXMLRPCCommand(getCmd("d.get_custom"), [$hash, "chk-state"]),
            new rXMLRPCCommand(getCmd("d.get_custom"), [$hash, "chk-time"]),
            new rXMLRPCCommand(getCmd("d.get_custom"), [$hash, "chk-stime"]),
    ...
    private static function collectTorrentPaths($torrent)
    {
        ...
        if (isset($info['files']) && is_array($info['files'])) {
            ...
            $path = self::makeSafeRelativePath([$info['name']]);
            if ($path === null) {
                return(null);
            }
            $paths[] = $path;
    ...
    private static function makeSafeRelativePath($components)
    {
        $normalized = [];
    ...
    private static function removeEmptySubFolders($path, $baseAbs)
    {
        $dir = dirname($path);
        ...
        while ($dir !== $baseAbs && strpos(FileUtil::addslash($dir), $basePrefix) === 0) {
            ...
            $scanned = @scandir($dir);
            if (!is_array($scanned) || count(array_diff($scanned, ['.', '..'])) || !@rmdir($dir)) {
                return;
            }
            $dir = dirname($dir);
        }
    ...
    private static function cleanupObsoleteFiles($oldTorrent, $newTorrent, $baseDir)
    {
        if (empty($baseDir)) {
            return;
        }
    ...
    private static function existingViews()
    {
    ...
    private static function buildReplacementAddition($connectionSeed, $throttle, $ratioViews, $existingViews, $state, $marker)
    {
        $now = time();
        $addition = [
     ...
     */
    private static function waitForLoad($hash, $marker)
    {
        for ($attempt = 0; $attempt < self::LOAD_WAIT_ATTEMPTS; $attempt++) {
            ...
            $req = new rXMLRPCRequest(new rXMLRPCCommand(
                getCmd("d.get_custom"),
                [$hash, self::REPLACEMENT_MARKER_KEY],
            ));
            $req->important = false;
    ...
    private static function eraseStaged($hash)
    {
    ...
    private static function restoreExistingTorrent($hash, $wasOpen, $wasStarted)
    {
    ...
    private static function activateReplacement($hash, $wasOpen, $wasStarted)
    {
        if (!$wasOpen && !$wasStarted) {
            return(true);
        }
    ...
    private static function clearReplacementMarker($hash)
    {
        $req = new rXMLRPCRequest(new rXMLRPCCommand(
            getCmd("d.set_custom"),
            [$hash, self::REPLACEMENT_MARKER_KEY, ""],
    ...
    public static function createTorrent($torrent, $hash)
    {
        ...
        if ($exists === true) {
            ...
            $markerReq = new rXMLRPCRequest([
                new rXMLRPCCommand(getCmd("d.get_custom"), [$newHash, self::REPLACEMENT_MARKER_KEY]),
    ...
    private static function appendAnnounceUrls($value, &$urls)
    {
        if (is_array($value)) {
    ...
    public static function run($hash, $state = null, $time = null, $successful_time = null, $label = null)
    {
        global $ignoreLabels;
