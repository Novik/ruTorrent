...
class rRSS
{
                ...
                if (($ret!==false) && ($ret>0)) {
                    foreach ($items[2] as $rssItem) {
                        ...
                        if (array_key_exists('enclosure', $item)) {
                            ...
                            $href = $item['guid'];
                        } elseif (array_key_exists('source', $item)) {
                            $href = $item['source'];
                        }
                        $guid = $href;
                        if (array_key_exists('guid', $item) && preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $item['guid'])) {
                            $guid = $item['guid'];
                        } elseif (array_key_exists('link', $item) && preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $item['link'])) {
                            $guid = $item['link'];
                        }
    ...
    public static function removeTegs($s)
    {
        $last = '';
        while ($s!=$last) {
            $last = $s;
