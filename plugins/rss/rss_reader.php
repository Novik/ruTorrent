...
class RegexRSSReader
{
	...
	private function nextNode($name, $ctx = null)
	{
		...
		if ($parentName && str_starts_with($name, 'text()')) {
			// find text in of node
			$regex = '<' . $parentName . '.*?>\s*((?P<value><!\[CDATA\[.*?\]\]>)|(?P<value2>.*?))<\/' . $parentName . '>';
		} elseif ($parentName && str_starts_with($name, '@')) {
			// find attr in of node
			...
			if ($ctx && str_ends_with($ctx['value'], '/>')) {
				// self closing tag
				return null;
			}
