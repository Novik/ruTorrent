...
class XMPPHP_XMLStream {
	...
	/**
	 * Set context for socket
	 *
	 * @param string $conntype
	 * @param array $opts
	 */
	public function set_context($opts)
	{
	...
	public function connect($timeout = 30, $persistent = false, $sendinit = true) {
		...
		do {
			...
			try {
				$this->socket = @stream_socket_client("$conntype://{$this->host}:{$this->port}", $errno, $errstr, $timeout, $conflag, $this->context);
			} catch (Exception $e) {
