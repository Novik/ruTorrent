...
class rXmpp
{
        ...
        if (isset($HTTP_RAW_POST_DATA)) {
            ...
            foreach ($vars as $var) {
                ...
                if ($parts[0] == "jabberHost") {
                    $jabberHost = $parts[1];
                } elseif ($parts[0] == "jabberPort") {
                    $jabberPort = $parts[1];
                } elseif ($parts[0] == "jabberJid") {
                    $jid = explode("@", $parts[1]);
                    $this->jabberLogin = $jid[0];
                    $this->jabberServer = count($jid) > 1 ? $jid[1] : "";
                } elseif ($parts[0] == "jabberPasswd") {
                    $this->jabberPasswd = $parts[1];
                } elseif ($parts[0] == "useEncryption") {
                    $useEncryption = $parts[1];
                } elseif ($parts[0] == "advancedSettings") {
                    $this->advancedSettings = $parts[1];
                } elseif ($parts[0] == "jabberFor") {
                    $this->jabberFor = $parts[1];
                } elseif ($parts[0] == "message") {
                    if ($parts[1]) {
    ...
    public function setHandlers()
    {
        ...
        if ($this->message !== '' && isset($this->jabberServer) && isset($this->jabberLogin) && isset($this->jabberPasswd) && isset($this->jabberFor)) {
            $cmd = $theSettings->getOnFinishedCommand(['xmpp' . User::getUser(),
                getCmd('execute.nothrow') . '={sh,-c,\"$0\" \"$@\" </dev/null >/dev/null 2>&1 &,' . Utility::getPHP() . ',' . $pathToXmpp . '/notify.php,"$' . getCmd('d.name') . '=","' . User::getUser() . '"}',
            ]);
        } else {
            $cmd = $theSettings->getOnFinishedCommand(['xmpp' . User::getUser(), getCmd('cat=')]);
        }
        $req->addCommand($cmd);
        return($req->success());
    }
