...
class rUnpack
{
    ...
    protected static function processTask($task, $commands)
    {
        ...
        ? [
            ...
            "log" => [],
            "params" => [],
            "errors" => [ "Dry-run operation, no real task start" ],
    ...
    // php/utility/.
    protected static function quotaAllows()
    {
    ...
    {
        global $rootPath;
        $ret = [ "no" => -1, "pid" => 0, "status" => 255, "log" => [], "errors" => ["Unknown error."] ];

        if (!self::quotaAllows()) {
            $ret["errors"] = ["Quota limitation was reached. Unpack failed."];
            return($ret);
        }
        ...
        if (!empty($mode)) {
            $req = new rXMLRPCRequest(
                new rXMLRPCCommand("f.get_frozen_path", [$hash,intval($fileno)]),
            );
            if ($req->success()) {
                $filename = $req->val[0];
                if ($filename == '') {
            ...
            if ($req->success()) {
                ...
                $req = new rXMLRPCRequest(
                    new rXMLRPCCommand("f.multicall", [$hash,"",getCmd("f.get_path=")]),
                );
                if ($req->success()) {
                    $rarPresent = false;
                    $zipPresent = false;
    ...
    public function setHandlers()
    {
        global $rootPath;
        if ($this->enabled) {
            $cmd =  rTorrentSettings::get()->getOnFinishedCommand(['unpack' . User::getUser(),
            ...
            $cmd = rTorrentSettings::get()->getOnFinishedCommand(['unpack' . User::getUser(), getCmd('cat=')]);
        }
