...
if (isset($geoip2CityDb) && file_exists($geoip2CityDb)) {
    try {
        $cityReader = new \GeoIp2\Database\Reader($geoip2CityDb);
    } catch (\Exception $e) {
        $cityReader = null;
    }
}
if (isset($geoip2IspDb) && file_exists($geoip2IspDb)) {
    try {
        $ispReader = new \GeoIp2\Database\Reader($geoip2IspDb);
    } catch (\Exception $e) {
        $ispReader = null;
    }
}
            ...
            if (strlen($value)) {
                ...
                if ($retrieveHost) {
                    if ($dns) {
                        ...
                        if (substr($value, 0, 1) == '[') {
                            $a = '';
                            foreach (str_split(inet_pton(substr($value, 1, -1))) as $char) {
                                $a .= str_pad(dechex(ord($char)), 2, '0', STR_PAD_LEFT);
                            }
                ...
                $ret[] = [ "ip" => $value, "info" => [ "country" => $country, "host" => $host, "comment" => $comment ] ];
            }
        }
    }
    if ($dns) {
