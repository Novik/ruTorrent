...
class ipDB
{
    ...
    public static function trunc($str, $maxlength = 128)
    {
        $str = trim($str);
    ...
    public function add($ip, $comment)
    {
        ...
        if ($comment=='') {
            sqlite_exec(
                $this->handle,
                "delete from comments where ip=".$ip,
                $this->error
            );
        } else {
            ...
            if (sqlite_query1($this->handle, "select count(comment) from comments where ip=".$ip, $this->error)) {
                sqlite_exec1(
                    $this->handle,
                    "update comments set comment=".$comment." where ip=".$ip,
                    $this->error
                );
            } else {
                sqlite_exec1(
                    $this->handle,
                    "insert into comments (ip,comment) values (".$ip.",".$comment.")",
                    $this->error
                );
            }
        }
